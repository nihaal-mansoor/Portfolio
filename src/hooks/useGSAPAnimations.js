...
export const useSplitTextAnimation = (scope, dependencies = []) => {
  ...
  useGSAP(() => {
    const elements = containerRef.current?.querySelectorAll('.split-text') || [];
...
export const useCustomGSAP = (animationFunction, dependencies = [], options = {}) => {
  const containerRef = useRef(null);
...
export const useGSAPTimeline = (timelineFunction, dependencies = [], options = {}) => {
  const containerRef = useRef(null);
